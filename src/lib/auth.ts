 ...
 */
export async function createOrUpdateUserDocument(uid: string, email: string, nome: string, role: UserRole, senha?: string, cnpj?: string, telefone?: string, referredBy?: string): Promise<Usuario> {
  ...
  if (telefone) userDoc.telefone = telefone;
  if (referredBy && referredBy !== uid) userDoc.referredBy = referredBy;
